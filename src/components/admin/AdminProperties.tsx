          ...
          >
              ...
              {tabs.map((t) => (
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 400,
                    borderBottom: activeTab === t.key ? "2px solid #c8b8a2" : "2px solid transparent",
                  }}
                >
                  {t.label}
                </button>
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <FieldGroup label="Tipo">
                    <select
                      ...
                      style={{ borderColor: "rgba(0,0,0,0.1)", color: "#0a0a0a", backgroundColor: "#fff", fontWeight: 300 }}
                    >
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <div className="w-10 h-5 relative cursor-pointer" onClick={() => setEditingProp({ ...editingProp, activa: !editingProp.activa })}>
                    <div className="w-10 h-5 transition-colors" style={{ backgroundColor: editingProp.activa ? "#0a0a0a" : "rgba(0,0,0,0.1)" }} />
                    <div className="absolute top-0.5 w-4 h-4 transition-all" style={{ backgroundColor: "#fff", left: editingProp.activa ? "22px" : "2px" }} />
                  </div>
                  <span className="text-sm font-body" style={{ color: "#0a0a0a", fontWeight: 300 }}>Visible en el sitio</span>
                </label>
              </div>
            )}
              ...
              <div className="space-y-6">
                <FieldGroup label="Foto principal">
                  <div className="flex items-center gap-3">
                    <label className="cursor-pointer flex items-center gap-2 px-4 py-2 text-[11px] tracking-[0.1em] uppercase font-body border transition-colors hover:bg-gray-50" style={{ borderColor: "rgba(0,0,0,0.1)", color: "#0a0a0a", fontWeight: 400 }}>
                ...
                <FieldGroup label={`Fotos adicionales (${(editingProp.imagenes || []).length}/10)`}>
                  <label className="cursor-pointer flex items-center gap-2 px-4 py-2 text-[11px] tracking-[0.1em] uppercase font-body border transition-colors hover:bg-gray-50 w-fit" style={{ borderColor: "rgba(0,0,0,0.1)", color: "#0a0a0a", fontWeight: 400 }}>
                    <Upload className="h-3.5 w-3.5" />
            ...
            {activeTab === "detalles" && (
              <div className="space-y-5">
                ...
                <div>
                  <span className="block text-[11px] tracking-[0.1em] uppercase mb-3 font-body" style={{ color: "#9a9a9a", fontWeight: 400 }}>
      ...
      {confirmDelete && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-sm p-8 border" style={{ backgroundColor: "#fff", borderColor: "rgba(0,0,0,0.08)" }}>
            <h3 className="font-display text-lg mb-3" style={{ color: "#0a0a0a", fontWeight: 400 }}>¿Eliminar propiedad?</h3>
            <p className="text-sm font-body mb-6" style={{ color: "#9a9a9a", fontWeight: 300 }}>Esta acción no se puede deshacer. La propiedad y sus imágenes serán eliminadas permanentemente.</p>
      ...
      <div className="border" style={{ borderColor: "rgba(0,0,0,0.08)" }}>
        ...
        {propiedades.length === 0 ? (
          <div className="p-8 text-center">
            <p className="text-sm font-body" style={{ color: "#9a9a9a", fontWeight: 300 }}>No hay propiedades. Crea la primera.</p>
          </div>
        ) : (
          propiedades.map((p) => (
            <div
            ...
            >
              ...
              <span className="text-sm font-body truncate" style={{ color: "#0a0a0a", fontWeight: 400 }}>{p.titulo}</span>
              <span className="text-xs font-body" style={{ color: "#9a9a9a", fontWeight: 300 }}>{p.precio || "—"}</span>
              <span className="text-[10px] tracking-[0.1em] uppercase font-body px-2 py-0.5 inline-block w-fit" style={{ backgroundColor: p.tipo === "venta" ? "rgba(0,0,0,0.04)" : "rgba(200,184,162,0.15)", color: "#0a0a0a", fontWeight: 400 }}>{p.tipo}</span>
              <span className="text-xs font-body" style={{ color: "#9a9a9a", fontWeight: 300 }}>{p.comuna || "—"}</span>
              <span className="text-xs font-body flex items-center gap-1" style={{ color: "#9a9a9a", fontWeight: 300 }}>
              ...
              <div>
                <button onClick={() => handleToggle(p.id, p.activa)} className="w-10 h-5 relative cursor-pointer">
                  <div className="w-10 h-5 transition-colors" style={{ backgroundColor: p.activa ? "#0a0a0a" : "rgba(0,0,0,0.1)" }} />
                  <div className="absolute top-0.5 w-4 h-4 transition-all" style={{ backgroundColor: "#fff", left: p.activa ? "22px" : "2px" }} />
                </button>
              </div>
              <button onClick={() => { setActiveTab("general"); setEditingProp(p); }} className="p-1.5 transition-colors hover:bg-gray-100 w-fit" style={{ color: "#9a9a9a" }}>
...
function FieldGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-[11px] tracking-[0.1em] uppercase mb-2 font-body" style={{ color: "#9a9a9a", fontWeight: 400 }}>{label}</label>
      {children}
    </div>
  );
}
