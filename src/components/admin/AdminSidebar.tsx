...
export function AdminSidebar({ activeSection, onChangeSection }: AdminSidebarProps) {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      <div className="p-6 border-b" style={{ borderColor: "rgba(0,0,0,0.08)" }}>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 flex items-center justify-center border" style={{ borderColor: "rgba(0,0,0,0.15)" }}>
            <span className="font-display text-sm tracking-widest" style={{ color: "#0a0a0a", fontWeight: 300 }}>PC</span>
          </div>
          <span className="text-[10px] tracking-[0.15em] uppercase font-body" style={{ color: "#0a0a0a", fontWeight: 400 }}>Admin</span>
        </div>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              style={{ backgroundColor: isActive ? "rgba(0,0,0,0.04)" : "transparent", color: isActive ? "#0a0a0a" : "#9a9a9a" }}
            >
      ...
      <div className="p-3 border-t" style={{ borderColor: "rgba(0,0,0,0.08)" }}>
        <Link to="/" className="flex items-center gap-3 px-3 py-2.5 mb-1 transition-colors" style={{ color: "#9a9a9a" }}>
          <Home className="h-4 w-4" />
          <span className="text-[13px] font-body" style={{ fontWeight: 300 }}>Ver sitio</span>
        </Link>
        <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors" style={{ color: "#9a9a9a" }}>
  ...
  return (
    <>
      ...
      <aside className="hidden md:block w-56 shrink-0 border-r h-screen sticky top-0" style={{ backgroundColor: "#f5f5f3", borderColor: "rgba(0,0,0,0.08)" }}>
        {sidebarContent}
      </aside>
    </>
  );
