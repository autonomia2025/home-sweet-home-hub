...
import { fetchPropiedadById, fetchConfiguracion } from "@/lib/supabase-helpers";
...
function PropiedadDetalle() {
  ...
  useEffect(() => {
    Promise.all([fetchPropiedadById(id), fetchConfiguracion()])
      .then(([p, c]) => {
        setProp(p);
        setConfig(c);
        // Dynamic SEO
        document.title = `${p.titulo} | Inmobiliaria Pérez-Campos`;
        const descMeta = document.querySelector('meta[name="description"]');
        if (descMeta) descMeta.setAttribute("content", `${p.descripcion || ""} — ${p.comuna || ""}`);
      })
      .catch(() => setError(true))
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#f5f5f3" }}>
      ...
      <div className="pt-24 pb-16 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="font-display text-2xl mb-8" style={{ color: "#0a0a0a", fontWeight: 400 }}>
            {prop.precio}
          </p>
          ...
          {allImages.length > 0 ? (
                  ...
                  {allImages.map((img, i) => (
                    <button
                      ...
                      style={{
                        ...
                        border: i === activeImg ? "2px solid #c8b8a2" : "2px solid transparent",
                      }}
                    >
            ...
            <div
              ...
              style={{ aspectRatio: "16/9", backgroundColor: "#e8e8e5" }}
            >
              <span className="font-display text-4xl" style={{ color: "#ccc", fontWeight: 300 }}>
                PC
              </span>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-12">
            ...
            <div>
              ...
              {hasChars && (
                <div className="mb-8" style={{ borderTop: "1px solid rgba(0,0,0,0.06)", paddingTop: "2rem" }}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {chars.superficie_m2 != null && (
                      <CharItem icon={<Ruler className="h-4 w-4" />} label="Superficie" value={`${chars.superficie_m2} m²`} />
                    )}
                    {chars.dormitorios != null && (
                      <CharItem icon={<BedDouble className="h-4 w-4" />} label="Dormitorios" value={String(chars.dormitorios)} />
                    )}
                    {chars.banos != null && (
                      <CharItem icon={<Bath className="h-4 w-4" />} label="Baños" value={String(chars.banos)} />
                    )}
                    {chars.estacionamiento != null && (
                      <CharItem icon={<Car className="h-4 w-4" />} label="Estacionamiento" value={chars.estacionamiento ? "Sí" : "No"} />
                    )}
                    {chars.bodega != null && (
                      <CharItem icon={<Package className="h-4 w-4" />} label="Bodega" value={chars.bodega ? "Sí" : "No"} />
                    )}
                  </div>
                </div>
              )}
              ...
              {prop.ubicacion_referencia && (
                <div className="flex items-center gap-2 mb-8">
                  ...
                  <span className="font-body text-sm" style={{ color: "#6a6a6a", fontWeight: 300 }}>
                    {prop.ubicacion_referencia}
                  </span>
                </div>
              )}
            ...
            <div>
              <div
                className="sticky top-24 p-6 border"
                style={{ borderColor: "rgba(0,0,0,0.08)", backgroundColor: "#fff" }}
              >
                <p className="font-display text-2xl mb-1" style={{ color: "#0a0a0a", fontWeight: 400 }}>
                  {prop.precio}
                </p>
                <span
                ...
                <p className="text-xs font-body text-center mb-5" style={{ color: "#9a9a9a", fontWeight: 300 }}>
...
function CharItem({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3 py-2">
      ...
      <div>
        <span className="block text-[10px] tracking-[0.1em] uppercase font-body" style={{ color: "#9a9a9a", fontWeight: 400 }}>
          {label}
        </span>
        <span className="text-sm font-body" style={{ color: "#0a0a0a", fontWeight: 400 }}>
          {value}
        </span>
      </div>
    </div>
...
function LoadingSkeleton() {
  return (
    <div className="min-h-screen pt-24 px-6" style={{ backgroundColor: "#f5f5f3" }}>
      <div className="max-w-6xl mx-auto">
        <Skeleton className="h-4 w-24 mb-8" />
        <Skeleton className="h-6 w-20 mb-4" />
        <Skeleton className="h-12 w-3/4 mb-3" />
        <Skeleton className="h-4 w-32 mb-8" />
        <Skeleton className="w-full mb-12" style={{ aspectRatio: "16/9" }} />
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-12">
          <div>
  ...
  return (
    ...
    >
      <div className="text-center">
        <h1 className="font-display text-3xl mb-4" style={{ color: "#0a0a0a", fontWeight: 300 }}>
          Esta propiedad no está disponible
        </h1>
        <p className="font-body text-sm mb-8" style={{ color: "#9a9a9a", fontWeight: 300 }}>
          Es posible que haya sido removida o que el enlace sea incorrecto.
        </p>
        <Link
          to="/"
          className="inline-block text-[11px] tracking-[0.15em] uppercase font-body py-3 px-6 border transition-all duration-300 hover:bg-[#0a0a0a] hover:text-white"
          style={{ borderColor: "#0a0a0a", color: "#0a0a0a", fontWeight: 400 }}
        >
          Volver al inicio
        </Link>
